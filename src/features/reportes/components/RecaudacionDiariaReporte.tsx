...
import { HiCalendar, HiCurrencyDollar, HiCollection, HiDownload, HiRefresh } from 'react-icons/hi';
import { obtenerRecaudacionDiariaApi } from '../api/reportesApi';
import { exportRecaudacionDiariaToExcel } from '../utils/exportToExcel';
import type { RecaudacionDiariaData } from '../types/reportes.types';
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white dark:bg-dark-card px-4 py-3 rounded-xl shadow-xl border border-neutral-100 dark:border-dark-border">
        <p className="text-sm font-semibold text-neutral-900 dark:text-white">{data.metodoPago || data.name}</p>
        <p className="text-sm text-neutral-600 dark:text-neutral-400">
          {formatMoney(data.total || data.value)}
        </p>
...
export const RecaudacionDiariaReporte: React.FC = () => {
  const [fecha, setFecha] = useState<string>(() => {
    const today = new Date();
    return today.toISOString().split('T')[0];
  });
  const [data, setData] = useState<RecaudacionDiariaData | null>(null);
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const response = await obtenerRecaudacionDiariaApi(fecha);
      if (response.success) {
  ...
  const chartData = useMemo(() => {
    if (!data?.desglosePorMetodoPago) return [];
    return data.desglosePorMetodoPago.map((item, index) => ({
      ...item,
  ...
  const handleExport = () => {
    if (data) {
      exportRecaudacionDiariaToExcel(data);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-neutral-900 dark:text-white">Recaudación Diaria</h2>
            <p className="text-sm text-neutral-500 dark:text-neutral-400">Total recaudado en el día seleccionado</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <input
              ...
              className="px-4 py-2 pr-10 border border-neutral-200 dark:border-dark-border rounded-xl text-sm bg-white dark:bg-dark-card text-neutral-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
            />
          </div>
          <button
            onClick={fetchData}
      ...
      {data && !loading && !error && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-5 text-white shadow-lg shadow-blue-500/30">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-sm font-medium opacity-90">Fecha</span>
              </div>
            ...
            <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-5 text-white shadow-lg shadow-emerald-500/30">
              ...
              <p className="text-2xl font-bold">{formatMoney(data.totalRecaudado)}</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-violet-500 to-purple-600 rounded-2xl p-5 text-white shadow-lg shadow-violet-500/30">
              ...
              <p className="text-2xl font-bold">{data.cantidadTransacciones}</p>
            </div>
          </div>
          ...
          {chartData.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white dark:bg-dark-card rounded-2xl border border-neutral-200 dark:border-dark-border shadow-sm overflow-hidden">
                <div className="px-6 py-5 border-b border-neutral-100 dark:border-dark-border">
                  <h3 className="font-semibold text-neutral-900 dark:text-white">Distribución por Método de Pago</h3>
                  ...
                  <div className="flex flex-wrap justify-center gap-4 mt-4">
                    {chartData.map((item, index) => (
                      <div key={index} className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-neutral-600 dark:text-neutral-400">{item.metodoPago}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-dark-card rounded-2xl border border-neutral-200 dark:border-dark-border shadow-sm overflow-hidden">
                <div className="px-6 py-5 border-b border-neutral-100 dark:border-dark-border">
                  <h3 className="font-semibold text-neutral-900 dark:text-white">Monto por Método de Pago</h3>
            ...
            <div className="bg-white dark:bg-dark-card rounded-2xl border border-neutral-200 dark:border-dark-border p-12 text-center">
              <div className="w-16 h-16 rounded-full bg-neutral-100 dark:bg-dark-bg flex items-center justify-center mx-auto mb-4">
                <HiCollection className="w-8 h-8 text-neutral-400" />
              </div>
              <p className="text-neutral-500 dark:text-neutral-400">No hay transacciones registradas para esta fecha</p>
            </div>
          )}
          ...
          {chartData.length > 0 && (
            <div className="bg-white dark:bg-dark-card rounded-2xl border border-neutral-200 dark:border-dark-border shadow-sm overflow-hidden">
              <div className="px-6 py-5 border-b border-neutral-100 dark:border-dark-border">
                <h3 className="font-semibold text-neutral-900 dark:text-white">Desglose Detallado</h3>
              </div>
                ...
                <table className="w-full">
                    ...
                    {chartData.map((item, index) => (
                      <tr key={index} className="hover:bg-neutral-50 dark:hover:bg-dark-hover transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-3 h-3 rounded-full" style={{ backgroundColor: item.color }} />
                            <span className="font-medium text-neutral-900 dark:text-white">{item.metodoPago}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400">
                            {((item.total / data.totalRecaudado) * 100).toFixed(1)}%
                          </span>
                        </td>
                      </tr>
                    ))}
                  ...
                  <tfoot className="bg-neutral-50 dark:bg-dark-bg">
                    <tr>
                      ...
                      <td className="px-6 py-4 text-right font-semibold text-neutral-900 dark:text-white">
                        {data.cantidadTransacciones}
                      </td>
                      <td className="px-6 py-4 text-right font-bold text-emerald-600 dark:text-emerald-400">
                        {formatMoney(data.totalRecaudado)}
                      </td>
                      <td className="px-6 py-4 text-right">
